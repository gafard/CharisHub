...
export default function CommunityGroupChat({
  ...
  const lastMessageKey = useMemo(() => {
    ...
    return last ? `${last.id}:${last.updated_at}:${last.created_at}` : 'empty';
  }, [messages]);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`charishub_group_chat_${groupId}`)
      .on(
        'postgres_changes',
        {
          event: '*',
        ...
        (payload) => {
          ...
          if (payload.eventType === 'INSERT') {
            ...
            if (cleanUuid(newPost.group_id || '') !== cleanedGroupId) return;
            ...
            if (cleanUuid(updatedPost.group_id || '') !== cleanedGroupId) return;
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`charishub_group_chat_presence_${groupId}`, {
      config: {
        presence: {
          key: actor.deviceId,
        },
  ...
  useEffect(() => {
    const markAsRead = () => {
      if (!messages.length || !actor.deviceId || document.visibilityState !== 'visible') return;
  ...
  const startRecording = useCallback(async () => {
    ...
    try {
      ...
      setFeedback('Impossible de démarrer le micro.');
    }
  ...
  const onPickFile = useCallback(
    (file?: File | null) => {
      if (!file) return;
      if (editingMessageId) {
        setFeedback('Vous modifiez un message : pièce jointe désactivée.');
        return;
      }
  ...
  const onStartEditMessage = useCallback(
    (messageId: string) => {
      ...
      void loadMessages(); // Refresh to ensure we have latest version
    },
    [actor.deviceId, clearAttachment, loadMessages, parsedById]
  );
              ...
              <div className="inline-flex items-center gap-1 text-[10px] font-medium text-[color:var(--foreground)]/50">
                ...
                {mine ? (
                  ...
                  >
                    ...
                    {seenCount > 0 ? `Vu ${seenCount}` : 'Envoyé'}
                  </span>
                ) : null}
              </div>
            </div>
              ...
              <div className="flex items-start gap-2">
                <div className="min-w-0 flex-1">
                  <VoiceNotePlayer
                    ...
                    title={attachmentLabel || 'Note vocale prête à envoyer'}
                    compact
                  />
                </div>
                <button
        ...
        <div className="fixed inset-0 z-[100] grid place-items-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-xs rounded-[32px] bg-white p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="mt-6 flex flex-col gap-2">
              <button
                onClick={() => void confirmDeleteMessage(deleteConfirmationId)}
                className="w-full rounded-2xl bg-rose-600 py-3 text-sm font-bold text-white shadow-lg active:scale-[0.97] transition-transform"
              >
                Oui, supprimer
              </button>
              <button
