...
const REFLECTION_QUESTIONS = [
  { id: 'q1', text: 'Que révèle ce chapitre sur Dieu ou sur Jésus ?' },
  { id: 'q2', text: 'Qu’est-ce qui me frappe, me dérange ou m’éclaire ici ?' },
  { id: 'q3', text: 'Y a-t-il un appel concret pour ma vie aujourd’hui ?' },
  { id: 'q4', text: 'Quelle vérité dois-je retenir ou méditer davantage ?' },
];
...
function getFocusContext(
  ...
  if (!fallbackReading || typeof fallbackChapter !== 'number') {
    return null;
  }
...
export default function ReflectionQuestions({
  ...
  const [expandedQuestion, setExpandedQuestion] = useState<string | null>(
    REFLECTION_QUESTIONS.find((question) => !(initialAnswers[question.id] ?? '').trim())?.id ?? REFLECTION_QUESTIONS[0]?.id ?? null,
  );
  ...
  const answeredCount = REFLECTION_QUESTIONS.filter((question) => (answers[question.id] ?? '').trim().length > 0).length;
  const chapterComplete = answeredCount > 0;
  const promptCount = DAILY_PROMPTS.filter((prompt) => (dailyAnswers[prompt.id] ?? '').trim().length > 0).length;
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="grid shrink-0 gap-2 text-right">
            <span className="inline-flex items-center justify-center rounded-full border border-white/10 bg-[rgba(255,255,255,0.05)] px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.18em] text-[rgba(255,240,222,0.76)]">
              {answeredCount}/{REFLECTION_QUESTIONS.length} réponses
            </span>
            {finalChapter ? (
              <span className="inline-flex items-center justify-center rounded-full border border-white/10 bg-[rgba(255,255,255,0.05)] px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.18em] text-[rgba(255,240,222,0.76)]">
          ...
          return (
            ...
            >
              ...
              >
                ...
                >
                  {hasAnswer ? <CheckCircle2 size={16} /> : index + 1}
                </div>
