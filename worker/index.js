...
self.addEventListener('push', function (event) {
  ...
  try {
    ...
    const title = data.title || 'Formation Biblique';
    const body = data.body || 'Nouveau contenu disponible';
    const icon = data.icon || '/globe.svg';
    const badge = data.badge || '/globe.svg';
    const tag = data.tag || 'default';
    const url = data.url || data.data?.url || '/groups';
    ...
    try {
      ...
      event.waitUntil(
        self.registration.showNotification('Formation Biblique', {
          body: text,
          icon: '/globe.svg'
        })
      );
